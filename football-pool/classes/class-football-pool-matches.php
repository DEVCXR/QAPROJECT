...
class Football_Pool_Matches {
	...
	private function match_info() {
		return array_filter( $this->all_matches, function( $v, $k ) {
	...
	public function get_match_info_for_user( $user_id, $match_ids = array(), $all_matches = false ) {
		...
		foreach ( $matches as $row ) {
			...
			$match_template_params = array(
				...
				'home_team' => isset( $teams->team_names[(int) $row['home_team_id']] ) ?
								Football_Pool_Utils::xssafe( $teams->team_names[(int) $row['home_team_id']] ) : '',
				'away_team' => isset( $teams->team_names[(int) $row['away_team_id']] ) ?
								Football_Pool_Utils::xssafe( $teams->team_names[(int) $row['away_team_id']] ) : '',
			...
			$match_template_params = apply_filters( 'footballpool_match_table_match_template_params', $match_template_params, $row['id'] );
		...
		foreach ( $matches as $row ) {
			...
			$match_template_params = array(
				...
				'home_team' => isset( $teams->team_names[ (int) $info['home_team_id'] ] ) ?
								Football_Pool_Utils::xssafe( $teams->team_names[(int) $info['home_team_id']] ) : '',
				'away_team' => isset( $teams->team_names[ (int) $info['away_team_id'] ] ) ?
								Football_Pool_Utils::xssafe( $teams->team_names[(int) $info['away_team_id']] ) : '',
