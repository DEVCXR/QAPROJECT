...
class Football_Pool {
	private static $pages = array(
					array( 'slug' => 'tournament', 'title' => 'Partidos', 'comment' => 'closed' ),
						array( 'slug' => 'teams', 'title' => 'Equipos', 'parent' => 'tournament', 'comment' => 'closed' ),
						array( 'slug' => 'groups', 'title' => 'Grupos', 'parent' => 'tournament', 'comment' => 'closed' ),
		...
		if ( ! is_admin() ) {
			...
			wp_localize_script( 'pool-js'
				...
				, array(
					...
					'do_ajax_saves' => ( FOOTBALLPOOL_FRONTEND_AJAX === true ) ? 'yes' : 'no',
	...
	public static function activate( $action = 'install' ) {
		...
		$options['show_save_messages'] = 0; // defaults to false
		...
		if ( $action === 'install' ) {
			...
			if ( Football_Pool_Utils::get_fp_option( 'keep_data_on_uninstall', 0, 'int' ) === 0 ) {
				...
				$wpdb->query( $sql );
				// default ranking
				$sql = $wpdb->prepare( "INSERT INTO `{$prefix}rankings` ( `id`, `name`, `user_defined` ) VALUES ( %d, %s, 0 );"
										, FOOTBALLPOOL_RANKING_DEFAULT
										, __( 'default ranking', 'football-pool' )
								);
				$wpdb->query( $sql );
				// default season
				$sql = $wpdb->prepare( "INSERT INTO `{$prefix}seasons` ( `id`, `name` ) VALUES ( %d, %s );"
	...
	public static function get_locale() {
		return apply_filters( 'plugin_locale', get_locale(), 'football-pool' );
	}
